...
using Payeer.Api.Exceptions;
using Payeer.Api.Json;

namespace Payeer.Api.Web
{
    internal sealed class PayeerApiGateway : IPayeerApiGateway
    {
        ...
        public PayeerApiGateway(IHttpClientFactory clientFactory)
        {
            ...
            {
                ContractResolver = new SpecialContractResolver(),
                Converters = new List<JsonConverter> { new PayeerEntityJsonConverter() }
            };
        }
        ...
        {
            var httpClient = _clientFactory.CreateClient(PayeerClient.HttpClientName);
            var response = await httpClient.SendAsync(httpRequestMessage);

            if (!response.IsSuccessStatusCode)
            {
                throw new PayeerApiException(response.StatusCode, response.Content);
            }
